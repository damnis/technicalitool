...
def search_ticker_fmp(query): query = query.upper().strip() url = f"https://financialmodelingprep.com/api/v3/search?query={query}&limit=50&apikey={FMP_API_KEY}" try: response = requests.get(url) data = response.json() if not data: return [] def beurs_score(exchange): if exchange == "NASDAQ": return 0 if exchange == "NYSE": return 1 return 2 data.sort(key=lambda x: beurs_score(x.get("exchangeShortName", ""))) return [(item["symbol"], item.get("name", item["symbol"])) for item in data] except Exception as e: st.error(f"❌ Fout bij zoeken naar tickers: {e}") return []
...
df = pd.DataFrame(data["historical"])
    ...
    if not ticker.upper().endswith("-USD"):  # Geen crypto
        try:
            if ticker.upper().endswith(".AS"):
                cal = mcal.get_calendar("XAMS")  # Euronext Amsterdam
            else:
            ...
            df = df[df.index.date.astype("O").isin(valid_days)]
            st.write("✅ Na filtering:", len(df))
        except Exception as e:
            st.error(f"❌ Kalenderfout: {e}")
    else:
        st.write("🪙 Crypto: geen beursdagenfilter toegepast (7/7)")

    # ➕ Voeg OHLC toe (vereenvoudigd voor lijn-data)
